...
const JWTLogin = () => {
  ...
  const checkAndRegisterDefaultAdmin = async () => {
    try {
      const admins = await creator_backend.getAdmins();
      if (admins.length === 0) {
        await creator_backend.addAdmin(2580, 'admin', 'admin', 'admin123');
  ...
  const handleLogin = async (values, { setSubmitting }) => {
    ...
    try {
      ...
      const isAdmin = await creator_backend.adminLogin(username, password);
      const isClient = await creator_backend.clientLogin(username, password); // Updated to "clientLogin"
      ...
      if (isAdmin && isAdmin.length > 0) {
        ...
        const clientUser = { id: isClient[0].id.toString(), role: 'client' }; // Updated role to "client"
        localStorage.setItem('user', JSON.stringify(clientUser)); // Store user in localStorage
        window.location.href = `${window.location.origin}/client/dashboard`; // Updated path
      } else {
  ...
  const signInWithInternetIdentity = async () => {
    ...
    const client = await creator_backend.getClientByPrincipal(principal); // Updated method
  
    console.log(client);
  
    if (client) {
      const clientUser = { id: client[0].id.toString(), role: 'client' }; // Updated role to "client"
      localStorage.setItem('user', JSON.stringify(clientUser)); // Store user in localStorage
      window.location.href = `${window.location.origin}/client/dashboard`; // Updated path
    } else {
