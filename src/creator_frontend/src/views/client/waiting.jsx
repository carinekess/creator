...
const WaitingPage = () => {
    ...
    const checkPaymentStatus = async () => {
      try {
        ...
        if (paymentResponse) {
          ...
          const customerName = 'Artwork Purchaser'; // Updated customer name
          const customerPhone = paymentResponse.tel;

          if (paymentStatus === 'success') {
            // Update the status in the backend
            const updateStatusResult = await creator_backend.updatePaymentStatus(
              tx_ref, 
              transactionId,
              'success'
            );
            if (updateStatusResult) {
              ...
              // Send confirmation SMS to the purchaser
            ...
            // Update the status in the backend
            await creator_backend.updatePaymentStatus(
              tx_ref, 
              transactionId, 
              'failed'
            );
  ...
  // Function to send SMS to the purchaser
  const sendMessage = async (customerName, customerPhone, transactionId, amount, tx_ref) => {
    const formData = new FormData();
    formData.append('sender_id', 'L7-IT');
    formData.append('ref', 'sms');
    formData.append('message', `Dear ${customerName}, your payment for the artwork has been received. Transaction ID: ${transactionId}, Transaction reference: ${tx_ref}, Amount: ${amount} Rwf. Thank you!`);
                ...
                <Card.Body className="text-center">
                  ...
                  {status === 'failed' && (
                    <Alert variant="danger">
                      Payment failed. 
                      <Button onClick={() => navigate('/client/view-artworks')} className="ml-2">Try Again</Button>
                    </Alert>
                  )}
                </Card.Body>
              </Col>
